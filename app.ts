...
const validatePin = async (userPin: any) => {
  const { pin } = await inquirer.prompt([
...
const handleTransferMoney = async (userPin: any) => {
  if (!(await validatePin(userPin))) return;
...
const handleReceiveMoney = async (userPin: any) => {
  if (!(await validatePin(userPin))) return;
...
const handleBillPayments = async (userPin: any) => {
  if (!(await validatePin(userPin))) return;
...
const handleEasyLoadBundles = async (userPin: any) => {
  if (!(await validatePin(userPin))) return;
...
const handleMobilePackages = async (userPin: any) => {
  if (!(await validatePin(userPin))) return;
...
// Handle cash back
const handleCashBack = async (userPin: any) => {
  if (!(await validatePin(userPin))) return;
...
// Handle savings
const handleSavings = async (userPin: any) => {
  if (!(await validatePin(userPin))) return;
